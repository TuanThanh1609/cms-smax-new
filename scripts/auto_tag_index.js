...
for (const page of pagesToTag) {
  ...
  const processElement = (el, type, isImg = false) => {
    ...
    const newKey = `${prefix}-auto-${type}-${tagCounts[type]}`;
...
console.log(`Total new tags found for index: ${allNewRecords.length}`);
fs.writeFileSync(path.join(__dirname, 'index_new_tags.json'), JSON.stringify(allNewRecords, null, 2));
